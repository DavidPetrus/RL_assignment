...
#Multiple environments giving shit - all subsequent code changed -> where 'envs' would have appeared - replaced with env 
#def make_env(i):
#    def _thunk():
...
#I'm not sure I completely understand the purpose of this function
...
state = env.reset()
state = state.flatten()#State space flattened to get right shape and run
...
while frame_idx < max_frames:
    ...
    for _ in range(num_steps):
        ...
        log_prob = dist.log_prob(actioni).unsqueeze(0)
        ...
        states.append(state)
        actions.append(actioni.unsqueeze(0))
    ...
    #This will only run if the number of steps taken equals the mini_batch size - error gets kick by PPO_iter function
